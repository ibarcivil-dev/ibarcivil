  ...
  return (
    <div className={styles.river}>
      <div>
        ...
        <h1 className={styles.riverTitle} style={{ borderBottom: 'none', marginBottom: '8px', paddingBottom: '0' }}>
          {topic.name}
        </h1>
        <p style={{
          fontFamily: 'var(--font-sans)',
          ...
          return (
            <div key={art.id} className={styles.riverItem}>
              <div className={styles.riverItemContent}>
                <span className={styles.riverItemTag}>{art.isPerspective ? 'Perspective' : 'Essay'}</span>
                <h2 className={styles.riverItemTitle}>
                  <Link href={`/article/${art.slug}`}>{art.title}</Link>
                </h2>
                <p className={styles.riverItemExcerpt}>{art.excerpt}</p>

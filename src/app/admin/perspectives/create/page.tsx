...
export default function CreatePerspective() {
  const router = useRouter();
  
  // Auto-generated ID for this new perspective instance
  ...
  const triggerSave = async () => {
    ...
    try {
      ...
      if (!hasSavedOnce) {
        ...
        router.replace(`/admin/perspectives/${articleId}`);
      } else {
    ...
    <div style={{ fontFamily: 'var(--font-sans)', color: 'var(--text-primary)', minHeight: '100vh', backgroundColor: 'var(--background)' }}>
      ...
      <div style={drawerStyle}>
        ...
        <form onSubmit={handleManualSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px', flexGrow: 1 }}>
          ...
          >
            <Save size={16} />
            <span>{isSaving ? 'Saving...' : 'Publish Perspective'}</span>
          </button>
        </form>
      </div>

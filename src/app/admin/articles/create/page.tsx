...
export default function CreateArticle() {
  ...
  // Auto-generated ID for this new article instance
  const [articleId] = useState(() => {
    if (typeof window !== 'undefined' && window.crypto && window.crypto.randomUUID) {
      return window.crypto.randomUUID();
    }
    return '00000000-0000-0000-0000-000000000000'.replace(/[0-9]/g, () => Math.floor(Math.random() * 10).toString());
  });
  ...
  useEffect(() => {
    async function loadMetadata() {
      try {
        ...
        const { data: topData } = await supabase.from('topics').select('*').order('name');
        const { data: issData } = await supabase.from('issues').select('*').order('number', { ascending: false });

        if (authData) {
          setAuthors(authData);
          if (authData.length > 0) setAuthorId(authData[0].id);
        }
  ...
  return (
    <div style={{ fontFamily: 'var(--font-sans)', color: 'var(--text-primary)', minHeight: '100vh', backgroundColor: 'var(--background)' }}>
      ...
      {/* 4. Publishing Slide-Over Drawer */}
      <div 
      ...
      <div style={drawerStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)', paddingBottom: '16px' }}>
          <h2 style={{ margin: 0, fontFamily: 'var(--font-serif)', fontSize: '1.5rem', fontWeight: 500 }}>Article Settings</h2>
          <button 
            type="button" 
            onClick={() => setPublishingDrawerOpen(false)} 
        ...
        <form onSubmit={handleManualSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px', flexGrow: 1 }}>
          ...
          >
            <Save size={16} />
            <span>{isSaving ? 'Saving...' : 'Publish Essay'}</span>
          </button>
        </form>
      </div>
